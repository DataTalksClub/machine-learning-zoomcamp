...
The ingress and scaling rows above are the current checkpointed outputs. The
definitions target is intentionally still pending: its latest imagegen
candidate is visually crisp but renders `LOADBALANCER` and `CLUSTERIP` in all
caps, so it has not been accepted until a deterministic correction preserves
the exact required spellings `LoadBalancer` and `ClusterIP`.
