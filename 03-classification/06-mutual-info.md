...
```python
mutual_info_score(x, y) # Scikit-Learn class for calculating the mutual information
# between the x target variable and y feature.

df[x].apply(y) # apply a y function to the x series of the df dataframe.

df.sort_values(ascending=False).to_frame(name='x') # sort values in an ascending order
# and called the column as x.
```
